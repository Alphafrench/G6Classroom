...
-- This file contains the database structure for the assignment management system
...
CREATE PROCEDURE GetAssignmentStats(IN assignment_id INT)
BEGIN
    SELECT 
        -- Assignment details
        a.title,
...
-- Procedure to calculate letter grade from percentage
DELIMITER //

CREATE FUNCTION CalculateLetterGrade(percentage DECIMAL(5,2))
RETURNS VARCHAR(2)
DETERMINISTIC
BEGIN
...
-- Insert sample assignments (assuming admin user exists with ID 1)
...
('HTML/CSS Fundamentals Project', 'Create a responsive website using HTML5 and CSS3', 'Build a complete responsive website that includes: 1) Homepage with navigation 2) About page 3) Contact form 4) Responsive design 5) Valid HTML5 and CSS3', 1, 'project', 100.00, '2024-12-15 23:59:59', TRUE, 1, TRUE, '["html", "css", "js", "zip"]'),
('JavaScript Quiz 1', 'Basic JavaScript concepts and syntax', 'Complete the quiz covering variables, functions, arrays, and objects. You have 30 minutes to complete.', 1, 'quiz', 50.00, '2024-12-10 16:00:00', FALSE, 1, FALSE, NULL),
('Database Design Essay', 'Design a database for an e-commerce system', 'Write a 1000-word essay describing your database design choices, entity relationships, and normalization considerations.', 1, 'essay', 75.00, '2024-12-20 23:59:59', TRUE, 1, TRUE, '["doc", "docx", "pdf", "txt"]')
...
CREATE INDEX idx_assignments_teacher_active ON assignments(teacher_id, is_active);
CREATE INDEX idx_submissions_assignment_status ON assignment_submissions(assignment_id, status);
CREATE INDEX idx_grades_assignment_score ON assignment_grades(assignment_id, percentage);
...
-- Get student submission status for all assignments
-- SELECT s.*, a.title, a.due_date, u.username as student_name
-- FROM assignment_submissions s
-- JOIN assignments a ON s.assignment_id = a.id
-- JOIN users u ON s.student_id = u.id
...
--     MIN(percentage) as minimum_score,
--     MAX(percentage) as maximum_score,
